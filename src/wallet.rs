// src/wallet.rs - Wallet implementation with Hybrid PQC support
...
use pqcrypto_dilithium::dilithium3::{
    detached_sign, SecretKey as PqcSecretKey, PublicKey as PqcPublicKey, keypair
};
use pqcrypto_traits::sign::{SecretKey as PqcSecretKeyTrait, PublicKey as PqcPublicKeyTrait, DetachedSignature as PqcDetachedSignatureTrait};
...
impl Wallet {
    ...
    pub fn load_from_file(node_config: &NodeConfig) -> Result<Self> {
        ...
        let pqc_sk = PqcSecretKey::from_bytes(&decoded.pqc_secret_key).map_err(|_| anyhow!("PQC SK load fail"))?;
        let pqc_pk = PqcPublicKey::from_bytes(&decoded.pqc_public_key).map_err(|_| anyhow!("PQC PK load fail"))?;
